...
class WaypointEditor:
    ...
    - 変更をCSVファイルに保存する。
    """
    # 【変更点】初期化メソッドに another_filepath を追加
    def __init__(self, master, filepath, another_filepath=None):
        self.master = master
        self.filepath = filepath
        # 【変更点】2つ目のファイルパスをインスタンス変数として保存
        self.another_filepath = another_filepath
        ...
        # --- プロットの初期化 ---
        self.points = self.ax.scatter(self.df['x'], self.df['y'], c='blue', picker=5, s=10)
        ...
        # 【変更点】渡されたファイルパスを使ってCSVを読み込む
        if self.another_filepath:
            try:
                # ヘッダーなし、1列目(0)をx, 2列目(1)をyとして読み込む
                another_path_df = pd.read_csv(
                    self.another_filepath, # ハードコーディングされていたパスを置き換え
                    header=None,
                    usecols=[0, 1],
                    names=['x', 'y']
                )
                self.ax.plot(
                 ...
                 messagebox.showwarning("警告", f"{self.another_filepath} が見つかりませんでした。")
            except Exception as e:
                messagebox.showerror("エラー", f"{self.another_filepath} の読み込みに失敗しました: {e}")
...
def main():
    # 【変更点】ここでCSVファイルのパスをまとめて指定します
    main_csv_path = "traj.csv"
    another_csv_path = "0.csv" # 2つ目のCSVファイルのパス

    root = tk.Tk()
    # 【変更点】WaypointEditorクラスに両方のパスを渡す
    app = WaypointEditor(root, main_csv_path, another_csv_path)
    root.mainloop()
